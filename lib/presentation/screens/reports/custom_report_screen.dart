    ...
    await showDialog(
      ...
      builder: (context) {
        return _FilterDialog(
          ...
          onApply: (showSales, showPurchases, showExpenses, sortBy, sortAscending) {
            setState(() {
  ...
  void _handleExport(ExportType type, Map<String, dynamic> data) {
    switch (type) {
      case ExportType.print:
        context.read<ReportsBloc>().add(
          PrintReportEvent('custom', data),
        );
        break;
      case ExportType.share:
        context.read<ReportsBloc>().add(
          ShareReportEvent('custom', data),
        );
        break;
      default:
        ScaffoldMessenger.of(context).showSnackBar(
