...
class SettingsBloc extends Bloc<SettingsEvent, SettingsState> {
  ...
  Future<void> _onLoadSettings(
    ...
    try {
      ...
      final themeMode =
          await _prefs.getString(StorageKeys.themeMode) ?? 'light';
      final isDark = themeMode == 'dark';
      final autoSyncEnabled =
          _prefs.getBool(StorageKeys.autoSyncEnabled) ?? false;
      ...
      // إذا كان النسخ التلقائي مفعلاً، نبدأ جدولة النسخ هنا عند الإقلاع
      if (autoBackupEnabled) {
        try {
          await _backupService.scheduleAutoBackup();
          _logger.info('تم جدولة النسخ الاحتياطي التلقائي أثناء التحميل');
        } catch (e) {
          _logger.warning(
            'فشل جدولة النسخ التلقائي أثناء التحميل',
  ...
  Future<void> _onToggleAutoBackup(
    ...
    try {
      ...
      _logger.error(
        'فشل تبديل النسخ الاحتياطي التلقائي',
        error: e,
        stackTrace: s,
      );
      emit(
        SettingsError('فشل تبديل النسخ الاحتياطي التلقائي: ${e.toString()}'),
      );
    }
  }
